...
func NewRedisService(adapter redisAdapter.RedisAdapter) RedisService {
	return &redisService{adapter: adapter}
}
...
func (s *redisService) Set(ctx context.Context, key string, value interface{}, ttl time.Duration) error {
	...
	err := s.adapter.Client.Set(ctx, key, data, ttl).Err()
	if err != nil {
...
func (s *redisService) Get(ctx context.Context, key string) (string, error) {
	val, err := s.adapter.Client.Get(ctx, key).Result()
	if err == redis.Nil {
...
func (s *redisService) Del(ctx context.Context, keys ...string) error {
	err := s.adapter.Client.Del(ctx, keys...).Err()
	if err != nil {
...
func (s *redisService) Exists(ctx context.Context, key string) (bool, error) {
	val, err := s.adapter.Client.Exists(ctx, key).Result()
	if err == redis.Nil {
...
func (s *redisService) Expire(ctx context.Context, key string, ttl time.Duration) error {
	err := s.adapter.Client.Expire(ctx, key, ttl).Err()
	if err != nil {
